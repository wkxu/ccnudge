...
function buildDesktopNotifyCommand(message) {
  ...
  return `powershell -c "Start-Process -FilePath '${snorePath}' -ArgumentList '-t','CCNudge','-m','${message}','-silent' -WindowStyle Hidden"`;
}
...
async function testDesktopNotification() {
  ...
  try {
    await execAsync(`powershell -c "Start-Process -FilePath '${snorePath.replace(/\\/g, '\\\\')}' -ArgumentList '-t','CCNudge','-m','Test notification','-silent' -WindowStyle Hidden"`);
