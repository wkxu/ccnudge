...
async function setupNotification(event, soundPath, enableDesktopNotify = false) {
  ...
  if (!settings.hooks[event]) {
    ...
    // Remove existing ccnudge hook if any
    settings.hooks[event] = settings.hooks[event].filter(h => h.name !== 'ccnudge');
    settings.hooks[event].push(ccnudgeHookGroup);
  } else {
    settings.hooks[event] = [settings.hooks[event], ccnudgeHookGroup];
  }
  ...
  if (event) {
    ...
    for (const evt of configuredEvents) {
      if (Array.isArray(settings.hooks[evt])) {
        settings.hooks[evt] = settings.hooks[evt].filter(h => h.name !== 'ccnudge');
